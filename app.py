...
def load_models():
    ...
    cnn.load_state_dict(torch.load("trained_models/cnn_model.pt", map_location=device))
    cnn.to(device).eval()

    resnet = models.resnet50(weights=None)
    resnet.fc = nn.Linear(resnet.fc.in_features, len(class_names))
    resnet.load_state_dict(torch.load("trained_models/resnet_model.pt", map_location=device))
    resnet.to(device).eval()

    efficientnet = models.efficientnet_b0(weights=None)
    efficientnet.classifier[1] = nn.Linear(efficientnet.classifier[1].in_features, len(class_names))
    efficientnet.load_state_dict(torch.load("trained_models/efficientnet_model.pt", map_location=device))
    ...
    vit.load_state_dict(torch.load("trained_models/vit_model.pt", map_location=device))
    vit.to(device).eval()

    vgg = models.vgg16(weights=None)
    vgg.classifier[6] = nn.Linear(vgg.classifier[6].in_features, len(class_names))
    vgg.load_state_dict(torch.load("trained_models/vgg16_model.pt", map_location=device))
    vgg.to(device).eval()

    densenet = models.densenet121(weights=None)
    densenet.classifier = nn.Linear(densenet.classifier.in_features, len(class_names))
    densenet.load_state_dict(torch.load("trained_models/densenet_model.pt", map_location=device))
    ...
    convnext.load_state_dict(torch.load("trained_models/convnext_model.pt", map_location=device))
    ...
    beit.load_state_dict(torch.load("trained_models/beit_model.pt", map_location=device))
    beit.to(device).eval()
    ...
    deit.load_state_dict(torch.load("trained_models/deit_model.pt", map_location=device))
    deit.to(device).eval()

    # YOLO classification models
    yolo8 = YOLO("trained_models/yolo8.pt")
    yolo11 = YOLO("trained_models/yolo11.pt")
    yolo12 = YOLO("trained_models/yolo12.pt")
